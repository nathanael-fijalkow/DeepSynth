...
lexicon = list(range(-256, 256))  # all elements of a list must be from lexicon
...
if os.path.exists("./" + MODEL_NAME + "2.weights"):
    model.load_state_dict(torch.load("./" + MODEL_NAME + "2.weights"))
...
# Load all tasks
tasks = []
with open(subset, "r") as fd:
    raw_tasks = json.load(fd)
    for raw_task in raw_tasks:
        name = raw_task["program"]
        raw_examples = raw_task["examples"]
        examples = [(raw_example["inputs"], raw_example["output"]) for raw_example in raw_examples]
...
for task in tasks:
    ...
    examples = [(list2env(i), o) for (i, o) in examples]
    dataset.append((name, grammar, make_checker(dsl, examples)))
print("Filtered tasks:", len(dataset))
...
for algo_index in range(len(list_algorithms)):
    ...
    filename = f"./algo_{algo_name}_model_{MODEL_NAME}_dataset_{datataset_name}_{subset_name}_results_semantic.pickle"
