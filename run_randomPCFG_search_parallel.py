...
def insert_prefix_toprog(prefix, prog, target_type):
    ...
    # print("Reconstructing:", prefix, "(", type(
    # prefix[0]), ")", "prog:", prog, "(", type(prog), ")")
...
def run_algorithm_parallel(pcfg: PCFG, algo_index: int, splits: int,
    ...
    def bounded_generator(prefix, cur_pcfg, i):
        if algorithm in reconstruct:
            ...
            def new_gen():
                ...
                p = next(gen)
                probability = pcfg.probability_program(
                    pcfg.start, p if prefix is None else insert_prefix_toprog(
    ...
    return [[search_times[i], cumulative_probabilities[i]] for i in range(len(search_times))]
...
def create_dataset():
    ...
    countpoints = np.linspace(
        start=0, stop=max_number_programs, num=number_countpoints)

    for splits in [2, 6]:
        r_time = np.zeros(
            (number_samples, number_algorithms, number_timepoints))
        r_program = np.zeros(
            (number_samples, number_algorithms, number_countpoints))
        ...
        for j, algo_index in enumerate(range(number_algorithms)):
            ...
            with open('results_syntactic/cumulative_probability_vs_time_{}_{}_splits{}.csv'.format(name_algo, timeout, splits), 'w', encoding='UTF8', newline='') as f:
                writer = csv.writer(f)
                header = ['search time', 'mean cumulative probability',
                          'standard deviation']
                writer.writerow(header)
                for x, t in enumerate(timepoints):
                    writer.writerow(
                        (t, result_time_mean[j][x], result_time_std[j][x]))

            with open('results_syntactic/cumulative_probability_vs_number_programs_{}_{}_splits{}.csv'.format(name_algo, timeout, splits), 'w', encoding='UTF8', newline='') as f:
                writer = csv.writer(f)
                header = ['number of programs',
                          'mean cumulative probability', 'standard deviation']
                writer.writerow(header)
                for x in range(number_countpoints):
                    writer.writerow(
                        (x, result_program_mean[j][x], result_program_std[j][x]))
...
def plot_cumulative_probability_vs_number_programs():
    ...
    for algo_index in range(len(list_algorithms)):
        ...
        color = sc.get_facecolors()[0].tolist()
        plt.fill_between(countpoints, result_top, result_low,
                         facecolor=color, alpha=0.2)
